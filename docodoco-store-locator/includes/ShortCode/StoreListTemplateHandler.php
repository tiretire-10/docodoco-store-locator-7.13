...
class StoreListTemplateHandler {
     ...
     * 店舗一覧ページテンプレートの読み込み
     */
    public function store_list_template_include($atts) {        
        // ショートコードの属性を取得
        $atts = shortcode_atts(array(
            'category' => '',  // デフォルトは全カテゴリー
        ), $atts);


// デバッグ用（一時的に追加）
error_log('Category from shortcode: ' . $atts['category']);
        ...
        if (!empty($atts['category'])) {
            ...
            "SELECT * FROM {$wpdb->prefix}docosl_stores 
             WHERE publish_status = %d AND category = %s 
...
// デバッグ用（一時的に追加）
error_log('Number of stores found: ' . count($stores));
